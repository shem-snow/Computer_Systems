...
dictionary_t *make_dictionary(int compare_mode, free_proc_t free_value) {
  ...
  dict->compare_mode = compare_mode;
  dict->free_value = (free_value ? free_value : no_free);
...
void *dictionary_get(dictionary_t *d, const char *key) { // Get the value of this key
  int i;
...
void *dictionary_value(dictionary_t *d, size_t i) { // Get value at this index
  return d->values[i];
}
